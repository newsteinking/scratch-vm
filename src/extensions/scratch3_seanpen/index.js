...
class Scratch3PenSEANBlocks {
    ...
    _clampPenSize (requestedSize) {
        return MathUtil.clamp(
            requestedSize,
            Scratch3PenSEANBlocks.PEN_SIZE_RANGE.min,
            Scratch3PenSEANBlocks.PEN_SIZE_RANGE.max
        );
    }
     ...
     */
    _getPenState (target) {
        let penState = target.getCustomState(Scratch3PenSEANBlocks.STATE_KEY);
        if (!penState) {
            penState = Clone.simple(Scratch3PenSEANBlocks.DEFAULT_PEN_STATE);
            target.setCustomState(Scratch3PenSEANBlocks.STATE_KEY, penState);
        }
        return penState;
    }
    ...
    _onTargetCreated (newTarget, sourceTarget) {
        if (sourceTarget) {
            const penState = sourceTarget.getCustomState(Scratch3PenSEANBlocks.STATE_KEY);
            if (penState) {
                newTarget.setCustomState(Scratch3PenSEANBlocks.STATE_KEY, Clone.simple(penState));
                if (penState.penDown) {
...
module.exports = Scratch3PenSEANBlocks;
